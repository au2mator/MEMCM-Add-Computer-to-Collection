...
# Type: New Service
#
# Title: SCCM - Add Computer to Collection
#
# v 1.0 Initial Release
#
#
# Init Release: 25.05.2021
# Last Update: 25.05.2021
# Code Template V 1.3
#
# URL: 
# Github: https://click.au2mator.com/MEMCM_AddComputerToCollection
#################
...
[string]$LogPath = "C:\_SCOworkingDir\TFS\PS-Services\SCCM - Add Computer to Collection"
[string]$LogfileName = "Add Computer to Collection"
...
try {
    Write-au2matorLog -Type INFO -Text "Get Computer from AD"
    $ADComputer = Get-ADComputer -Identity $c_Computer

    Write-au2matorLog -Type INFO -Text "Find  Computer in SCCM"
    $CMComputer = Get-WmiObject -ComputerName $SiteServer -Namespace "root\SMS\site_$CMSiteCode" -Class "SMS_R_System" -Filter "Name = '$($($ADComputer.Name).replace('\','\\'))'" | select name, resourceid
    Write-au2matorLog -Type INFO -Text "Found the Computer from CM: $($CMComputer.ResourceID)"  
    
    try {
        Write-au2matorLog -Type INFO -Text "Try to Add Computer to Collection $($c_Collection)"

        $coll = [wmi]"\\$($SiteServer)\$("root\SMS\site_$CMSiteCode"):SMS_Collection.CollectionID='$c_Collection'"	
        $ruleClass = [WMICLASS]"\\$($SiteServer)\$("root\SMS\site_$CMSiteCode"):SMS_CollectionRuleDirect"	
        [array]$rules = $null

        $newRule = $ruleClass.CreateInstance() 	
        $newRule.RuleName = $($CMComputer.name)
        $newRule.ResourceClassName = "SMS_R_System"	
        $newRule.ResourceID = $($CMComputer.resourceid)
        $rules += $newRule

        Write-au2matorLog -Type INFO -Text "Add Direct Rule to Collection $($coll.Name)"  
        $coll.AddMembershipRules($rules) #| Out-Null
        
        Write-au2matorLog -Type INFO -Text "Refresh Collection Membership $($coll.Name)"
        ...
        $au2matorReturn = "Computer $($CMUser.name) added to Collection $($coll.Name)"
        $AdditionalHTML = "Computer $($CMUser.name) added to Collection $($coll.Name)
            <br>
                "
        $Status = "COMPLETED"
        ...
        Write-au2matorLog -Type ERROR -Text "Error on adding to Computer to Collection, Error: $Error"       
    
        $au2matorReturn = "Error on adding to Computer to Collection, Error: $Error"
        $AdditionalHTML = "Error on adding to Computer to Collection
            <br>
            Error: $Error
                "
        $Status = "ERROR"
    ...
    Write-au2matorLog -Type ERROR -Text "Error to get User from SCCM, Error: $Error"

    $au2matorReturn = "Error to get User from SCCM, Error: $Error"
    $AdditionalHTML = "Error to get User from SCCM
        <br>
        Error: $Error
            "
    $Status = "ERROR"
